...
const Layout: React.FC<LayoutProps> = ({ children, activeView, setActiveView }) => {
    ...
    <>
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar pb-10">
        {currentMenuItems.map((item, idx) => {
          // Add logical separators for SuperAdmin
  ...
  return (
    <div className="min-h-screen flex bg-slate-50">
      ...
      <aside className={`fixed top-0 bottom-0 left-0 z-[70] w-72 bg-white flex flex-col shadow-2xl transition-transform duration-300 lg:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarContent />
      </aside>
      ...
      <main className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarOpen ? 'lg:ml-64' : 'lg:ml-20'}`}>
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-30">
          <div className="flex items-center gap-2 md:gap-4">
            <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 lg:hidden"><Menu size={20} /></button>
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="hidden lg:block"><Menu size={20} /></button>
            <h2 className="text-sm md:text-lg font-semibold text-slate-800 truncate">{currentMenuItems.find(m => m.id === activeView)?.label || 'Sector'}</h2>
          </div>
          
          <div className="flex items-center gap-3 md:gap-6">
             {currentUser.role === UserRole.STUDENT && (
               <div className="relative" ref={notifRef}>
                 <button onClick={() => setIsNotifOpen(!isNotifOpen)} className="p-2.5 bg-slate-50 text-slate-500 rounded-xl relative">
                   <Bell size={20} />
                   {unreadCount > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-black flex items-center justify-center rounded-full ring-2 ring-white">{unreadCount}</span>}
                 </button>
                 {isNotifOpen && (
                   <div className="absolute right-0 mt-3 w-80 md:w-96 bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden z-50">
                     <div className="p-5 bg-indigo-600 text-white flex justify-between items-center"><h4 className="font-black text-xs uppercase">Notifications</h4></div>
                     <div className="max-h-96 overflow-y-auto">
                        {myNotifs.length > 0 ? myNotifs.map(notif => (
                          <div key={notif.id} onClick={() => !notif.isRead && markNotificationRead(notif.id)} className={`p-5 border-b border-slate-50 cursor-pointer ${notif.isRead ? 'bg-white opacity-60' : 'bg-indigo-50/30'}`}>
                            <p className="text-sm font-black mb-1">{notif.title}</p>
                            <p className="text-xs text-slate-500">{notif.message}</p>
                          </div>
                        )) : <div className="p-10 text-center text-slate-300"><p className="text-xs font-black uppercase">No Alerts</p></div>}
                     </div>
                   </div>
                 )}
               </div>
             ...
             <div className="flex items-center gap-3">
                ...
                <div className="w-10 h-10 rounded-xl bg-indigo-100 flex items-center justify-center text-indigo-600 font-black border-2 border-white shadow-sm overflow-hidden">
