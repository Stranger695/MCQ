             ...
             {currentUser.role === UserRole.STUDENT && (
               <div className="relative" ref={notifRef}>
                 <button 
                   ...
                   className="p-2.5 bg-slate-50 text-slate-500 rounded-xl hover:bg-indigo-50 hover:text-indigo-600 transition-all relative"
                 >
                   <Bell size={20} />
                   {unreadCount > 0 && (
                     <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-black flex items-center justify-center rounded-full ring-2 ring-white animate-in zoom-in-50">
                       {unreadCount}
                     </span>
                   )}
                 </button>
                 ...
                 {isNotifOpen && (
                   <div className="absolute right-0 mt-3 w-80 md:w-96 bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-in slide-in-from-top-2 duration-200 z-50">
                     <div className="p-5 border-b border-slate-50 flex items-center justify-between bg-indigo-600 text-white">
                        <h4 className="font-black text-xs uppercase tracking-widest">Notification Hub</h4>
                        <span className="text-[10px] font-bold opacity-80">{myNotifs.length} Alerts</span>
                     </div>
                        ...
                        {myNotifs.length > 0 ? (
                          myNotifs.map(notif => (
                            <div 
                              ...
                              className={`p-5 border-b border-slate-50 transition-colors cursor-pointer relative group ${notif.isRead ? 'bg-white opacity-60' : 'bg-indigo-50/30'}`}
                            >
                              {!notif.isRead && <div className="absolute left-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-indigo-600 rounded-full"></div>}
                              <div className="flex items-start justify-between gap-4">
                                 <div>
                                    <p className={`text-sm font-black mb-1 ${notif.isRead ? 'text-slate-600' : 'text-slate-900'}`}>{notif.title}</p>
                          ...
                          <div className="p-10 text-center text-slate-300">
                             ...
                             <p className="text-[10px] font-black uppercase tracking-widest">Registry Clear</p>
                          </div>
                        )}
                     </div>
                   </div>
